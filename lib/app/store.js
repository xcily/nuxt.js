...
function getModule (filename) {
  ...
  if(module.actions){
    // ./main/article.js  =>  main/article/
    let namespace = filename.substring(2).replace('.js','/');
    if(namespace==='index/') namespace='';

    for(let action in module.actions){
      let actionFun = module.actions[action];
      module.actions[action] = async function(context, payload){
        let store = this;
        //console.log('this:',this)
        context.api = {
          get:async function (stateName,url) {
            await store.dispatch('apiGet',{
              namespace:namespace,
              url:url,
              state:stateName
            })
        ...
        await actionFun.apply(store, arguments);
      }
    }
  }
  return module
